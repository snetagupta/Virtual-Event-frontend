      ...
      <div className="flex items-center justify-between px-3 py-4 md:px-8 border-b-2">
        <div className="flex items-center justify-between w-full">
          ...
          <div className="hidden md:flex gap-6 mr-10  ">
            <NavLink to={"/past-events"} className={`${tagStyle} ${path == "/past-events" ? "underline decoration-primary text-primary" : "no-underline"} `}>
              Past Events
            </NavLink>
            <NavLink to={"/create-event"} className={`${tagStyle} ${path == "/create-event" ? "underline decoration-primary text-primary" : "no-underline"} `}>
              Create Event
            </NavLink>
            {/* <button className=' py-1 px-3 cursor-pointer text-base text-primary'>List your Events</button> */}
            <NavLink to={"/about-us"} className={`${tagStyle} ${path == "/about-us" ? "underline decoration-primary text-primary" : "no-underline"} `}>
              About us
            </NavLink>
          </div>
        </div>
          ...
          <div>
            {user ? (
              <div className="w-8 h-8 md:w-10 md:h-10">
                <img src="https://cdn3.pixelcut.app/1/3/profile_picture_1728ecf2bd.jpg" alt="" className="w-full h-full rounded-full ring-2 ring-primary object-cover cursor-pointer" onClick={() => setShowPofileMenu((prev) => !prev)} />
              </div>
            ) : (
